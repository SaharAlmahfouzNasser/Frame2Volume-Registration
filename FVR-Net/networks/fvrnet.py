...
class mynet3(nn.Module):
    ...
    def __init__(self, layers):
        ...
        ### modification ####
        self.fc1 = nn.Linear(100352,512)#nn.Linear(4096,512)#nn.Linear(2048, 512)
    ...
    def volBranch(self, vol):
        ...
        #print('vol shape after conv1',vol.shape)
        vol = self.bn1_vol(vol)
        #print('vol shape after bn1',vol.shape)
        vol = self.relu(vol)
        #print('vol shape after relu',vol.shape)


        vol = self.conv2_vol(vol)
        #print('vol shape after conv2',vol.shape)
        vol = self.relu(vol)
        #print('vol shape after relu',vol.shape)
    ...
    def frameBranch(self, frame):
        ...
        #print('conv2d_frame out shape',frame.shape)

        frame = frame.unsqueeze(1)
        #print('Frame shape after unsqueeze',frame.shape)

        frame = self.conv1_frame(frame)
        #print('Frame shape after conv1',frame.shape)
        frame = self.bn1_vol(frame)
        #print('Frame shape after bn1_vol',frame.shape)
        frame = self.relu(frame)
        #print('Frame shape after ReLU', frame.shape)
        # print('conv1 {}'.format(frame.shape))

        frame = self.conv2_frame(frame)
        #print('Frame shape after conv2_frame', frame.shape)
        frame = self.relu(frame)
        # print('conv2 {}\n'.format(frame.shape))
        #print('Frame shape out', frame.shape)
    ...
    def forward(self, vol, frame, device=None):
        ...
        if show_size:
            ...
            vol = self.volBranch(vol)
            frame = self.frameBranch(frame)
            #print('The shape of vol is ', vol.shape)
            #print('The shape of frame is',frame.shape)
            x = torch.cat((vol, frame), 2)
            ...
            mat = tools.dof2mat_tensor(input_dof=xx, device=device)
